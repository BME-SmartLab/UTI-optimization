...
Keras implementation of the UTI wedge representation of
...
def ultvid_mgc_generator(ultvid_files_all, mgc_scalers, batch_size, mode="train"):
    ultvid_batch = np.empty((batch_size, n_height, n_width, 1))
    mgc_batch = np.empty((batch_size, n_mgc))
    i_batch = 0
    epoch = 0
    # loop indefinitely
    while True:
        print('ultvid_mgc_seq_generator: loop start', mode)
        
        # randomize the order of files
        random.shuffle(ultvid_files_all)
        
        print('ultvid_mgc_seq_generator', mode, ultvid_files_all)
        
        # iterate through ult files
        for n in range(len(ultvid_files_all)):
            file = ultvid_files_all[n]
            print('\n ultvid_mgc_seq_generator: reading file ...', \
                str(int(100*n/len(ultvid_files_all)))+'%', mode, file, end='\r')
            
            ult_video = load_video_3D_fc1(file, framesPerSec)
            mgc_lsp_coeff = np.fromfile(file[:-15] + '_speech_volnorm_cut_ultrasound.mgclsp', dtype=np.float32).reshape(-1, n_mgc)
            
            # shuffle data for CNN
            permutation = np.random.permutation(ult_video.shape[0])
            np.take(ult_video, permutation, axis=0, out=ult_video)
            np.take(mgc_lsp_coeff, permutation, axis=0, out=mgc_lsp_coeff)
            
            for i in range(len(ult_video)):
                ult_img = ult_video[i]
                # crop only relevant area
                # 512 x 512
                ult_img = ult_img[39:551,467:979]
                # resize to smaller area
                ult_img = skimage.transform.resize(ult_img, (n_height, n_width), \
                    preserve_range=True, anti_aliasing=True, mode='constant')
                
                # white pixels to black
                # ult_img[ult_img == 0] = 255
                
                # debug save image
                if i == 100:
                    plt.figure(figsize=(10,10))
                    plt.imshow(ult_img, cmap='gray')
                    plt.title('minmax: ' + str(np.min(ult_img)) + '-' + str(np.max(ult_img)))
                    plt.savefig('img/' + os.path.basename(file[:-15]) + '_frame-100.png')
                    plt.close()
                
                # reshape for CNN
                ultvid_batch[i_batch] = ult_img.reshape(n_height, n_width, 1)
                mgc_batch[i_batch] = mgc_lsp_coeff[i]
                
                i_batch += 1
                
                if i_batch == batch_size:
                    # input data already between [0-1]
            ...
            if n == len(ultvid_files_all) - 1:
                print('\n\n ultvid_mgc_seq_generator: end of ultvid_files', mode, ', epoch ' + str(epoch+1) + '\n\n')
...
# raw size: 64 x 842 ~ 54k
# wedge resized and cut: 512 x 512 ~ 262k
...
for n_width in [512, 256, 128, 64, 32, 16, 8]:
    ...
    for speaker in speakers:
        ...
        if os.path.isdir(dir_data):
            for file in sorted(os.listdir(dir_data)):
                if file.endswith('_ultrasound.mp4'):
                    ultvid_files_all += [dir_data + file]
        ...
        # ultvid_files_all = ultvid_files_all[0:10]
        
        # train: first 90% of sentences
        ultvid_files_all_train = ultvid_files_all[0:int(0.9*len(ultvid_files_all))]
        # valid: last 10% of sentences
        ultvid_files_all_valid = ultvid_files_all[int(0.9*len(ultvid_files_all)):]
        
        # load all training data - only MGC-LSP
        for file in ultvid_files_all_train:
        ...
        for file in ultvid_files_all_valid:
            mgc_lsp_coeff = np.fromfile(file[:-15] + '_speech_volnorm_cut_ultrasound.mgclsp', \
                dtype=np.float32).reshape(-1, n_mgc)
        ...
        trainGen = ultvid_mgc_generator(ultvid_files_all_train, mgc_scalers, GENERATOR_BATCH, mode="train")
        validGen = ultvid_mgc_generator(ultvid_files_all_valid, mgc_scalers, GENERATOR_BATCH, mode="valid")
        ...
        model_name = 'models/UTIwedge_SSI_ResNet_datagen_' + \

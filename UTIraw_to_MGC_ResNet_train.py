...
Keras implementation of the UTI raw representation of
...
def ult_mgc_generator(ult_files_all, mgc_scalers, batch_size, mode="train"):
    ult_batch = np.empty((batch_size, n_lines, n_pixels_reduced, 1))
        ...
        for n in range(len(ult_files_all)):
            ...
            try:
                ...
                for i in range(len(ult_data)):
                    ...
                    ult_img = skimage.transform.resize(ult_img, (n_lines, n_pixels_reduced), \
                        preserve_range=True, anti_aliasing=True, mode='constant')
                    # reshape for CNN
                    ult_batch[i_batch] = ult_img.reshape(n_lines, n_pixels_reduced, 1)
...
for n_pixels_reduced in [842, 421, 210, 105, 53, 26, 13]:
    for speaker in speakers:
        ...
        if n_pixels_reduced in [842, 421]:
            GENERATOR_BATCH = 2
        else:
            GENERATOR_BATCH = 64
        ...
        # get ResNet model
        model = ResNet50_regression(input_shape = (n_lines, n_pixels_reduced, 1), n_output = n_mgc)
        ...
        model_name = 'models/UTIraw_SSI_ResNet_datagen_' + \
            str(n_lines) + 'x' + str(n_pixels_reduced) + '_' + speaker + '_' + current_date
